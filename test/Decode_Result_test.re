...
describe("Test value decoders", () => {
  ...
  test("String succeeds on string", () => expect(D.string(jsonString)) |> toEqual(Ok("Foo")));
  test("String fails on number", () => expect(D.string(jsonFloat)) |> toEqual(Error(Val(`ExpectedString, jsonFloat))));
  test("String fails on null", () => expect(D.string(jsonNull)) |> toEqual(Error(Val(`ExpectedString, jsonNull))));

  test("Float succeeds on float", () => expect(D.float(jsonFloat)) |> toEqual(Ok(1.4)));
  test("Float succeeds on int", () => expect(D.float(jsonInt)) |> toEqual(Ok(4.0)));
  test("Float fails on string", () => expect(D.float(jsonString)) |> toEqual(Error(Val(`ExpectedNumber, jsonString))));
  test("Float fails on null", () => expect(D.float(jsonNull)) |> toEqual(Error(Val(`ExpectedNumber, jsonNull))));

  test("Int succeeds on int", () => expect(D.int(jsonInt)) |> toEqual(Ok(4)));
  test("Int fails on float", () => expect(D.int(jsonFloat)) |> toEqual(Error(Val(`ExpectedInt, jsonFloat))));
  test("Int fails on string", () => expect(D.int(jsonString)) |> toEqual(Error(Val(`ExpectedNumber, jsonString))));
  test("Int fails on null", () => expect(D.int(jsonNull)) |> toEqual(Error(Val(`ExpectedNumber, jsonNull))));
});
...
describe("Test array decoders", () => {
  ...
  test("Array succeeds on array of string", () => expect(D.array(D.string, jsonArray)) |> toEqual(Ok([| "a", "b", "c" |])));
  test("Array string succeeds on empty array", () => expect(D.array(D.string, jsonEmptyArray)) |> toEqual(Ok([||])));
  test("Array int succeeds on empty array", () => expect(D.array(D.int, jsonEmptyArray)) |> toEqual(Ok([||])));
  test("Array fails on string", () => expect(D.array(D.string, jsonString)) |> toEqual(Error(Val(`ExpectedArray, jsonString))));
  test("Array inner decode int fails on array of string", () => expect(D.array(D.int, jsonArray)) |> toEqual(Error(Arr(decodeErrs))));
  test("Array fails on mixed array", () =>
    expect(D.array(D.string, jsonArrayMixed)) |> toEqual(Error(Arr(NonEmptyList.pure((2, Val(`ExpectedString, Js.Json.number(3.5)))))))
  );

  test("List succeeds on JSON array of string", () => expect(D.list(D.string, jsonArray)) |> toEqual(Ok(["a", "b", "c"])));
  test("List fails on JSON null", () => expect(D.list(D.string, Js.Json.null)) |> toEqual(Error(Val(`ExpectedArray, Js.Json.null))));
  test("List inner decode int fails on array of string", () => expect(D.list(D.int, jsonArray)) |> toEqual(Error(Arr(decodeErrs))));
});
...
describe("Test record field decoders", () => {
  ...
  test("String field succeeds", () => expect(D.field("name", D.string, obj)) |> toEqual(Ok("Foo")));
  test("Int field succeeds", () => expect(D.field("age", D.int, obj)) |> toEqual(Ok(30)));
  test("Field fails when missing", () => expect(D.field("blah", D.int, obj)) |> toEqual(Error(objPure("blah", MissingField))));
  test("Field fails on wrong type", () => expect(D.field("name", D.int, obj)) |> toEqual(Error(objPure("name", nameAsIntError))));
...
describe("Test optionally empty fields and values", () => {
  ...
  test("Trying to parse string field as int is Error, not None", () => expect(D.optionalField("s", D.int, obj)) |> toEqual(Error(Val(`ExpectedNumber, jsonString))));
});
