...
module Point = {
  ...
  let make = (x, y) => { x, y };
  let sample = Js.Dict.fromList([("x", number(3.1)), ("y", number(2.))]) |> object_;
};
...
module User = {
  ...
  let sampleComplete = Js.Dict.fromList([
    ("name", string("Foo")),
    ("age", number(30.)),
  ...
  let sampleSparse = Js.Dict.fromList([
    ("email", null)
  ]) |> object_;
};
...
describe("Test optional, fallback, hardcoded helpers", () => {
  ...
  test("Fallback ignored when field is present", () => expect(completeName) |> toEqual(Ok("Foo")));
  test("Fallback works when field is missing", () => expect(sparseName) |> toEqual(Ok("Bar")));

  test("Optional is Some when field is present", () => expect(completeAge) |> toEqual(Ok(Some(30))));
  test("Optional is None when field is missing", () => expect(sparseAge) |> toEqual(Ok(None)));
  test("Optional is None when value is JSON null", () => expect(sparseEmail) |> toEqual(Ok(None)));

  test("Hardcoded overrides value present in JSON", () => expect(completeNote) |> toEqual(Ok("No note")));
  test("Hardcoded is used when value is missing in JSON", () => expect(sparseNote) |> toEqual(Ok("No note")));
});
