...
module DecodeBase = (
  ...
  let string =
    value(Js.Json.decodeString, `ExpectedString);
  ...
  let list = (decode, json) =>
    array(decode, json) |> M.map(Array.to_list);

  let rec at = (fields, decode, json) => switch fields {
  | [] => decode(json)
  | [x, ...xs] =>
  ...
  module Pipeline = {
    ...
    let field = (name, decode) =>
      pipe(field(name, decode));

    let optionalField = (name, decode) =>
      pipe(optionalField(name, decode));

    let fallback = (name, decode, alt) =>
      pipe(fallback(name, decode, alt));
